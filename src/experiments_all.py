    ...
    if not read_file:
        ...
        try:
            ...
            for switch2_1_coef in (0.00001,):
                ...
                for switch1_2_coef in tqdm([0.000001 * (s ** i) for i in range(0, 50)]):
        ...
        try:
            ...
            for cor_coef in (1,):
                ...
                for queries_coef in tqdm([i / 25 for i in range(1, 51)]):
                    queries_matrices_1 = [matr * queries_coef for matr in copy.deepcopy(queries_matrices)]
...
def experiment_11(queueing_system: ColdReserveQueueingSystem, read_file=False):
    ...
    if not read_file:
        try:
            ...
            for repair_vect_elem in range(1):
                linux_check_cpu_temperature()
                if repair_vect_elem == 1:
                    local_queueing_system.set_PH_repair_stream(np.array([[0.2, 0.8]]), 6*np.array([[-1., 1.5], [4., -35.]]))
                ...
                for break_coef in tqdm([i for i in range(1, 101)]):
                    break_matrices_1 = copy.deepcopy(break_matrices)
                    break_matrices_1[0] *= break_coef
                    break_matrices_1[1] *= break_coef
                    local_queueing_system.set_MAP_break_stream(break_matrices_1[0], break_matrices_1[1])
