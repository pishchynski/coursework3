...
for j_experimental in (0.0001, 0.01, 1):
    ...
    P_neg_list = list()
    khi1_khi2_list = list()
    for i_experimental in range(1, 21):
        ...
        c_cor = np.sum(
            lamDb * np.dot(np.dot(theta, la.inv(-matrD_0)), matrD_1_ - np.dot(matrD_0, la.inv(-matrD_0)))) - 1 / c2var
        # print(c_cor)
        #
        ...
        matrA1 = np.array([[-1.86075, 0.], [0., -146.9994]]) * i_experimental ** 2 / 100
        L1 = matrA1.shape[0]
        L1_ = L1 + 1
